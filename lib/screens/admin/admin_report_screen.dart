...
class _AdminReportScreenState extends State<AdminReportScreen> {
  ...
  Future<void> _fetchFacultades() async {
    try {
      final response = await http.get(Uri.parse('http://192.168.1.51:3000/facultades'));
  ...
  Future<void> _fetchEscuelas(String facultad) async {
    ...
    try {
      final response = await http.get(Uri.parse('http://192.168.1.51:3000/escuelas?siglas_facultad=$facultad'));
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        setState(() {
  ...
  Future<void> _loadAsistencias() async {
    ...
    try {
      String url = 'http://192.168.1.51:3000/asistencias?';
      if (_dateRange != null) {
